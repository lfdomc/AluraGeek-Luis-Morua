...
const lista: HTMLUListElement = document.querySelector("[data-lista]")!;

// Interfaz del elemento ITEM con sus requisitos
interface Item {
    name: string;
    price: string;
    image: string;
...
export async function listarItems() {
    try {
        ...
        const listaAPI: Item[] = await conexionAPI.recibirDatosAPI();
